...
;(function($) {
	$.fn.twitterSearch = function(options) {
		...
		return this.each(function() {
			...
			var t = opts.titleLink ? ('<a href="../scripts/'+ opts.titleLink +'">'+ opts.title + '</a>') : ('<span>' + opts.title + '</span>');
			var $t = $(t);
			if (opts.titleLink)
				$t.css(opts.css['titleLink']);
			var $title = $('<div class="twitterSearchTitle"></div>').append($t).appendTo($frame).css(opts.css['title']);
			if (opts.bird) {
				var $b = $('<img class="twitterSearchBird" src="../scripts/'+opts.birdSrc+'" />').appendTo($title).css(opts.css['bird']);
				if (opts.birdLink)
					$b.wrap('<a href="../scripts/'+ opts.birdLink +'"></a>');
			}
	...
	$.fn.twitterSearch.formatter = function(json, opts) {
		var t = json.text;
		if (opts.anchors) {
			t = json.text.replace(/(http:\/\/\S+)/g, '<a href="../scripts/$1">$1</a>');
			t = t.replace(/\@(\w+)/g, '<a href="http://twitter.com/$1">@$1</a>');
		}
		var s = '<div class="twitterSearchTweet">';
		if (opts.avatar)
			s += '<img class="twitterSearchProfileImg" src="../scripts/' + json.profile_image_url + '" />';
